...
void *stream_compress_thread(void *_arg) {
    ...
    while(1) {
        ...
        if(qBuffer->fd.out > 0) {            
            int ret = write_buf(qBuffer->fd.out, (char *)&qBuffer->out_len, sizeof(qBuffer->out_len));
            if(ret < 0) {
            ...
            strcpy(errorMsg, "Out dump pipe not set.");
            goto thread_out;
        }
